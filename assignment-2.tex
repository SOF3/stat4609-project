...
While $d(i, j) < d(i, k)$ implies that $j$ is more simila to $i$ than $k$ is,
...
while the matrix in KNN does not contain negative value usually~\cite{Alpher01}.
Therefore, we define $$ d(i, j) = 1 - \rho_{ij} $$
